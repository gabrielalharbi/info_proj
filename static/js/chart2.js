...
var tip = d3.tip()
  ...
  .offset([-10, 0])
  .html(function(d) {
    return "<p><strong>County:</strong> <span style='color:orange'>" + d.county_name + "</span></p>" + "<p><strong>Male Population:</strong> <span style='color:orange'>" + d.male_population + "</span></p>"
    +"<p><strong>Female Population:</strong> <span style='color:orange'>" + d.female_population + "</span></p>";
  })
...
d3.csv("http://127.0.0.1:5002/templates/data/county_data.csv", type, function(error, data) {
    ...
    y.domain([0, d3.max(data, function(d) { if (isNaN(d.male_female_income)==false) return parseInt(d.male_female_income); })]);
